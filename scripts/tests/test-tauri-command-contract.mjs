...
function check() {
  ...
  if (commands.length !== 44) {
    fail(`registry must contain exactly 44 commands, got ${commands.length}`);
  }
  ...
  if (invokes.length !== 44) {
    fail(`desktop-api.ts must contain exactly 44 invoke wrappers, got ${invokes.length}`);
  }
...
if (failures.length > 0) {
  ...
  process.exitCode = 1;
    ...
    console.log("Tauri command contract passed: 44 commands, exact Rust/capability/permission/TypeScript sets.");
}
