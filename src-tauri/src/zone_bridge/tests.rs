...
fn custom_command_acl_and_frontend_path_boundary_are_explicit() {
    ...
    let tauri_contract: Value =
        serde_json::from_str(include_str!("../../../contracts/tauri-commands.v1.json")).unwrap();
    ...
    assert!(desktop_api.contains("extractActiveRunZoneAirStateDataset"));
    assert!(desktop_api.contains("cancelZoneResultDataset"));
    ...
    assert!(desktop_api.contains("saveStudioPreferences"));
    assert!(!desktop_api.contains("contamxPath"));
    assert!(!desktop_api.contains("simreadPath"));
    ...
    let setup_command = tauri_contract["commands"]
        .as_array()
        .and_then(|commands| {
            commands
                .iter()
                .find(|value| value["command"] == "save_studio_setup")
        })
        .expect("save_studio_setup contract");
    assert_eq!(setup_command["ts_wrapper"], "saveStudioPreferences");
    assert_eq!(
        setup_command["payload_keys"],
        serde_json::json!(["requestId", "language", "theme", "dataDirectory"])
    );
    let setup_start = release
        .find("pub fn save_studio_setup")
        .expect("save_studio_setup implementation");
    let setup_source = &release[setup_start..];
    let setup_end = setup_source
        .find("#[tauri::command]")
        .map(|index| &setup_source[..index])
        .unwrap_or(setup_source);
    assert!(!setup_end.contains("contamx_path"));
    assert!(!setup_end.contains("simread_path"));
...
fn valid_spatial_fixture_payload(active: &ActiveProjectContext) -> Value {
    json!({
        "schema_version": SPATIAL_SCHEMA_VERSION,
        "status": "available",
        "identity_sha256": active.baseline_source_sha256,
        ...
        "levels": [{
            ...
            "delta_height_unit": 0,
            "bounds": {"min_column": 10, "max_column": 10, "min_row": 20, "max_row": 20},
            "icons": [{
                "id": "icon-1",
                "icon_type": 5,
                "kind": "zone",
                "column": 10,
                "row": 20,
                "object_number": active.zones[0].contam_number,
                "binding": {
                    "kind": "zone",
                    "semantic_id": active.zones[0].zone_id,
                    "status": "bound",
                    "reason": null
                },
...
#[test]
fn spatial_payload_validator_accepts_verified_snapshot() {
    let fixture = primary_fixture();
    let project = execute_read(&fixture, "spatial-valid-read").result.unwrap();
    let active = active_context("spatial-valid", fixture, &project);
    let payload = valid_spatial_fixture_payload(&active);
    let snapshot = json!({
        "zones": [{"object_id": active.zones[0].zone_id}],
        "flow_paths": []
    });

    assert!(validate_spatial_projection(&payload, &active, &snapshot).is_ok());
    let _ = fs::remove_dir_all(active.draft_root);
}

#[test]
fn real_semantic_bridge_returns_validated_spatial_projection_without_writing_source() {
    let fixture = primary_fixture();
    let before = fs::read(&fixture).unwrap();
    let project = execute_read(&fixture, "spatial-real-read").result.unwrap();
    let active = active_context("spatial-real", fixture.clone(), &project);
    let request_id = "spatial-real-semantic";
    let request = json!({
        "protocol_version": PROTOCOL_VERSION,
    ...
    let envelope = execute_bridge_request(&request, request_id, READ_AND_PLAN_TIMEOUT).unwrap();
    assert!(envelope.ok);
    let snapshot = envelope.result.unwrap();
    assert_eq!(
        snapshot.get("result_type").and_then(Value::as_str),
        Some("semantic_project_snapshot")
    );
    let spatial = snapshot.get("spatial_projection").unwrap();
    validate_spatial_projection(spatial, &active, &snapshot).unwrap();
    assert_eq!(
        spatial.get("status").and_then(Value::as_str),
        Some("available")
    );
    assert_eq!(spatial["levels"][0]["icons"].as_array().unwrap().len(), 63);
...
#[test]
fn spatial_payload_validator_rejects_schema_identity_revision_and_forbidden_shape() {
    let fixture = primary_fixture();
    let project = execute_read(&fixture, "spatial-contract-read")
        .result
        .unwrap();
    let active = active_context("spatial-contract", fixture, &project);
    let snapshot = json!({"zones": [{"object_id": active.zones[0].zone_id}], "flow_paths": []});
    let valid = valid_spatial_fixture_payload(&active);

    for (field, value, expected) in [
        (
            "schema_version",
            json!("spatial_projection.v0"),
            "spatial_schema_version_invalid",
        ),
        (
            "identity_sha256",
            json!("0".repeat(64)),
        ...
        (
            "revision_id",
            json!("stale-revision"),
            "spatial_revision_mismatch",
        ),
        (
            "source_sha256",
            json!("1".repeat(64)),
            "spatial_source_mismatch",
        ),
    ] {
        let mut candidate = valid.clone();
        candidate[field] = value;
        assert_eq!(
            validate_spatial_projection(&candidate, &active, &snapshot)
                .unwrap_err()
                .code,
            expected
        );
    ...
    forbidden["levels"][0]["icons"][0]["source_path"] = json!("C:\\secret.prj");
    assert_eq!(
        validate_spatial_projection(&forbidden, &active, &snapshot)
            .unwrap_err()
            .code,
        "spatial_payload_invalid"
    );
    let _ = fs::remove_dir_all(active.draft_root);
}

#[test]
fn spatial_payload_validator_rejects_limits_duplicates_and_dangling_bindings() {
    let fixture = primary_fixture();
    let project = execute_read(&fixture, "spatial-limit-read").result.unwrap();
    let active = active_context("spatial-limit", fixture, &project);
    let snapshot = json!({"zones": [{"object_id": active.zones[0].zone_id}], "flow_paths": []});
    let valid = valid_spatial_fixture_payload(&active);

    let mut duplicate = valid.clone();
    duplicate["levels"][0]["icons"][0]["id"] = json!("icon-1");
    let repeated_icon = duplicate["levels"][0]["icons"][0].clone();
    duplicate["levels"][0]["icons"]
        .as_array_mut()
        .unwrap()
        .push(repeated_icon);
    assert_eq!(
        validate_spatial_projection(&duplicate, &active, &snapshot)
            .unwrap_err()
            .code,
    ...
    dangling["levels"][0]["icons"][0]["binding"]["semantic_id"] = json!("missing-zone");
    assert_eq!(
        validate_spatial_projection(&dangling, &active, &snapshot)
            .unwrap_err()
            .code,
        "spatial_dangling_binding"
    );

    let mut unavailable = valid.clone();
    unavailable["status"] = json!("unavailable");
    assert_eq!(
        validate_spatial_projection(&unavailable, &active, &snapshot)
            .unwrap_err()
            .code,
    ...
    overlong_name["levels"][0]["name"] = json!("界".repeat(171));
    assert_eq!(
        validate_spatial_projection(&overlong_name, &active, &snapshot)
            .unwrap_err()
            .code,
        "spatial_level_invalid"
    );

    let mut excessive_coordinate = valid.clone();
    excessive_coordinate["levels"][0]["icons"][0]["column"] = json!(MAX_SPATIAL_COORDINATE + 1);
    assert_eq!(
        validate_spatial_projection(&excessive_coordinate, &active, &snapshot)
            .unwrap_err()
            .code,
        "spatial_icon_invalid"
    );

    for (field, invalid_value) in [
        ("icon_type", json!(-1)),
        ("icon_type", json!(MAX_SPATIAL_ICON_TYPE + 1)),
        ("object_number", json!(MIN_SPATIAL_OBJECT_NUMBER - 1)),
        ("object_number", json!(MAX_SPATIAL_OBJECT_NUMBER + 1)),
    ] {
        let mut invalid_icon = valid.clone();
        invalid_icon["levels"][0]["icons"][0][field] = invalid_value;
        assert_eq!(
            validate_spatial_projection(&invalid_icon, &active, &snapshot)
                .unwrap_err()
                .code,
            "spatial_icon_invalid"
        );
    ...
    for (field, invalid_value, expected_code) in [
        ("level_number", json!(0), "spatial_level_number_invalid"),
        (
            "level_number",
            json!(MAX_SPATIAL_LEVEL_NUMBER + 1),
            "spatial_level_number_invalid",
        ),
        ("reference_height_unit", json!(-1), "spatial_level_invalid"),
        (
            "reference_height_unit",
            json!(MAX_SPATIAL_LEVEL_UNIT_CODE + 1),
            "spatial_level_invalid",
        ),
    ] {
        let mut invalid_level = valid.clone();
        invalid_level["levels"][0][field] = invalid_value;
        assert_eq!(
            validate_spatial_projection(&invalid_level, &active, &snapshot)
                .unwrap_err()
                .code,
            expected_code
        );
    ...
    let mut boundary_values = valid.clone();
    boundary_values["levels"][0]["level_number"] = json!(MAX_SPATIAL_LEVEL_NUMBER);
    boundary_values["levels"][0]["reference_height_unit"] = json!(MAX_SPATIAL_LEVEL_UNIT_CODE);
    boundary_values["levels"][0]["icons"][0]["icon_type"] = json!(MAX_SPATIAL_ICON_TYPE);
    boundary_values["levels"][0]["icons"][0]["column"] = json!(-MAX_SPATIAL_COORDINATE);
    boundary_values["levels"][0]["icons"][0]["row"] = json!(MAX_SPATIAL_COORDINATE);
    boundary_values["levels"][0]["bounds"] = json!({
        "min_column": -MAX_SPATIAL_COORDINATE,
        "max_column": -MAX_SPATIAL_COORDINATE,
        "min_row": MAX_SPATIAL_COORDINATE,
        "max_row": MAX_SPATIAL_COORDINATE
    });
    boundary_values["levels"][0]["icons"][0]["object_number"] = json!(MIN_SPATIAL_OBJECT_NUMBER);
    boundary_values["levels"][0]["icons"][0]["kind"] = json!("unknown");
    boundary_values["levels"][0]["icons"][0]["binding"] = json!({
        "kind": "none",
        "semantic_id": null,
        "status": "unbound",
        "reason": "spatial_unknown_icon_type"
    });
    let boundary_result = validate_spatial_projection(&boundary_values, &active, &snapshot);
    assert!(boundary_result.is_ok(), "{boundary_result:?}");

    let mut incompatible_binding = valid.clone();
    incompatible_binding["levels"][0]["icons"][0]["kind"] = json!("wall");
    assert_eq!(
        validate_spatial_projection(&incompatible_binding, &active, &snapshot)
            .unwrap_err()
            .code,
        "spatial_binding_invalid"
    );

    let mut oversized_payload = valid.clone();
    oversized_payload["forbidden_padding"] = json!("x".repeat(MAX_SPATIAL_PAYLOAD_BYTES));
    assert_eq!(
        validate_spatial_projection(&oversized_payload, &active, &snapshot)
            .unwrap_err()
            .code,
        "spatial_payload_limit_exceeded"
    );

    let mut too_many_levels = valid;
    too_many_levels["levels"] = Value::Array(
        (0..=MAX_SPATIAL_LEVELS)
            .map(|number| {
                json!({
    ...
    assert_eq!(
        validate_spatial_projection(&too_many_levels, &active, &snapshot)
            .unwrap_err()
            .code,
...
fn dataset_result_for_zone(
    active: &ActiveProjectContext,
    zone_index: usize,
    times: &[f64],
) -> ZoneAirStateResultView {
    let zone = &active.zones[zone_index];
    ZoneAirStateResultView {
        schema_version: RESULT_SCHEMA_VERSION.into(),
        result_type: "zone_air_state".into(),
        run_id: "run-1".into(),
        extraction_id: format!("extract-{}", zone.contam_number),
        zone_id: zone.zone_id.clone(),
        zone_number: zone.contam_number,
        zone_name: zone.name.clone(),
        source_line_number: zone.source_line_number,
        unit_system: "SI".into(),
        sample_count: times.len() as u64,
        samples: times
            .iter()
            .enumerate()
            .map(|(index, time)| ZoneAirStateSampleView {
                index: index as u64,
                day_of_year: 1,
                day_type: None,
                sim_time_seconds: *time,
                temperature_k: 293.15 + index as f64,
                reference_pressure_pa: -1.0 + index as f64,
...
fn dataset_requested(
    active: &ActiveProjectContext,
    count: usize,
) -> Vec<RequestedZoneIdentityView> {
    active
        .zones
        .iter()
        .take(count)
        .map(|zone| RequestedZoneIdentityView {
            zone_id: zone.zone_id.clone(),
            zone_number: zone.contam_number,
            zone_name: zone.name.clone(),
        })
...
#[test]
fn zone_result_dataset_is_deterministic_partial_and_uses_exact_common_times() {
    let fixture = primary_fixture();
    let project = execute_read(&fixture, "dataset-read").result.unwrap();
    let active = active_context("dataset", fixture, &project);
    let active_run = ActiveRunContext {
        project_session_id: active.project_session_id.clone(),
        source_sha256: active.source_sha256.clone(),
        revision_id: active.active_revision().revision_id.clone(),
        run_id: "run-1".into(),
        manifest_path: active.source_path.clone(),
        succeeded: true,
        summary: test_run_summary("run-1"),
    };
    let first_series = dataset_result_for_zone(&active, 0, &[0.0, 60.0, 120.0]);
    let second_series = dataset_result_for_zone(&active, 1, &[0.0, 120.0, 180.0]);
    let failure = ZoneResultFailureView {
        zone_id: active.zones[2].zone_id.clone(),
        zone_number: active.zones[2].contam_number,
        zone_name: active.zones[2].name.clone(),
        code: "simread_zone_failed".into(),
    };
    let first = build_zone_result_dataset(
        &active,
        &active_run,
        ZoneResultDatasetInput {
            extraction_batch_id: "batch-1",
            manifest_sha256: &"a".repeat(64),
            requested_zones: dataset_requested(&active, 3),
            successful_zone_series: vec![second_series.clone(), first_series.clone()],
            per_zone_failures: vec![failure.clone()],
            forced_status: None,
        },
    )
    .unwrap();
    let second = build_zone_result_dataset(
        &active,
        &active_run,
        ZoneResultDatasetInput {
            ...
            manifest_sha256: &"a".repeat(64),
            requested_zones: dataset_requested(&active, 3),
            successful_zone_series: vec![first_series, second_series],
            per_zone_failures: vec![failure],
            forced_status: None,
        },
    )
    .unwrap();
    ...
    assert_eq!(first.time_identity.kind, "exact_common");
    assert_eq!(first.time_identity.shared_time_seconds, vec![0.0, 120.0]);
    assert_eq!(first.evidence_summary.successful_zone_count, 2);
    assert_eq!(first.evidence_summary.failed_zone_count, 1);
    assert_eq!(first.dataset_fingerprint, second.dataset_fingerprint);
    assert_ne!(first.extraction_batch_id, second.extraction_batch_id);
    assert_eq!(first.bounds.total_samples, 6);
    let _ = fs::remove_dir_all(active.draft_root);
}

#[test]
fn zone_result_dataset_enforces_zone_and_sample_bounds_and_hard_failures() {
    let fixture = primary_fixture();
    let project = execute_read(&fixture, "dataset-bounds-read")
        .result
        .unwrap();
    let active = active_context("dataset-bounds", fixture, &project);
    let active_run = ActiveRunContext {
        project_session_id: active.project_session_id.clone(),
        source_sha256: active.source_sha256.clone(),
        revision_id: active.active_revision().revision_id.clone(),
    ...
    let zone = &active.zones[0];
    let too_many = (0..=MAX_RESULT_DATASET_ZONES)
        .map(|index| RequestedZoneIdentityView {
            zone_id: format!("zone-{index}"),
            zone_number: index as i64 + 1,
            zone_name: format!("Zone {index}"),
        })
        .collect();
    assert_eq!(
        build_zone_result_dataset(
            &active,
            &active_run,
            ZoneResultDatasetInput {
                extraction_batch_id: "batch",
                manifest_sha256: &"a".repeat(64),
                requested_zones: too_many,
                successful_zone_series: Vec::new(),
                per_zone_failures: Vec::new(),
                forced_status: None,
            },
        )
        .unwrap_err()
    ...
    let oversized_series = ZoneAirStateResultView {
        samples: vec![
            ZoneAirStateSampleView {
                index: 0,
                day_of_year: 1,
                day_type: None,
                sim_time_seconds: 0.0,
            ...
            MAX_RESULT_DATASET_SAMPLES + 1
        ],
        sample_count: (MAX_RESULT_DATASET_SAMPLES + 1) as u64,
        ..dataset_result_for_zone(&active, 0, &[0.0])
    };
    assert_eq!(
        build_zone_result_dataset(
            &active,
            ...
            ZoneResultDatasetInput {
                ...
                successful_zone_series: vec![oversized_series],
                per_zone_failures: Vec::new(),
                forced_status: None,
            },
        )
...
#[test]
fn result_dataset_cancellation_and_stale_retention_are_identity_bound() {
    let store = DesktopProjectSessionStore::default();
    store.begin_result_dataset("batch-1");
    assert!(!store.result_dataset_cancelled("batch-1"));
    store
        .result_dataset_cancel_requested
        .store(true, Ordering::Release);
    assert!(store.result_dataset_cancelled("batch-1"));
    assert!(!store.result_dataset_cancelled("old-batch"));
    store.finish_result_dataset("batch-1");
    assert!(!store.result_dataset_cancelled("batch-1"));
}

#[test]
fn last_trusted_result_dataset_requires_ready_or_successful_partial_data() {
    let fixture = primary_fixture();
    let project = execute_read(&fixture, "dataset-trust-read").result.unwrap();
    let active = active_context("dataset-trust", fixture, &project);
    let active_run = ActiveRunContext {
        project_session_id: active.project_session_id.clone(),
        source_sha256: active.source_sha256.clone(),
        revision_id: active.active_revision().revision_id.clone(),
    ...
    let ready = build_zone_result_dataset(
        &active,
        &active_run,
        ZoneResultDatasetInput {
            extraction_batch_id: "batch-ready",
            manifest_sha256: &"a".repeat(64),
            requested_zones: dataset_requested(&active, 1),
            successful_zone_series: vec![dataset_result_for_zone(&active, 0, &[0.0])],
            per_zone_failures: Vec::new(),
            forced_status: None,
        },
    )
    ...
    let store = DesktopProjectSessionStore::default();
    {
        ...
        state.active_project = Some(active.clone());
        state.active_run = Some(active_run.clone());
    }

    store
        .retain_result_dataset(&active, &active_run, ready.clone())
        .unwrap();
    assert_eq!(
        store
            .state
    ...
    partial.extraction_batch_id = "batch-partial".into();
    store
        .retain_result_dataset(&active, &active_run, partial)
        .unwrap();
    assert_eq!(
        store
            .state
    ...
    failed.successful_zone_series.clear();
    failed.evidence_summary.successful_zone_count = 0;
    failed.extraction_batch_id = "batch-failed".into();
    store
        .retain_result_dataset(&active, &active_run, failed)
        .unwrap();
    assert_eq!(
        store
            .state
    ...
    let mut empty_partial = ready;
    empty_partial.status = "partial";
    empty_partial.successful_zone_series.clear();
    empty_partial.evidence_summary.successful_zone_count = 0;
    empty_partial.extraction_batch_id = "batch-empty-partial".into();
    store
        .retain_result_dataset(&active, &active_run, empty_partial)
        .unwrap();
    assert_eq!(
        store
            .state
...
#[test]
fn ai_result_context_discloses_only_selected_exact_values_and_binds_fingerprint() {
    let fixture = primary_fixture();
    let project = execute_read(&fixture, "dataset-ai-read").result.unwrap();
    let active = active_context("dataset-ai", fixture, &project);
    let active_run = ActiveRunContext {
        project_session_id: active.project_session_id.clone(),
        source_sha256: active.source_sha256.clone(),
        revision_id: active.active_revision().revision_id.clone(),
    ...
    let dataset = build_zone_result_dataset(
        ...
        &active_run,
        ZoneResultDatasetInput {
            extraction_batch_id: "batch-ai",
            manifest_sha256: &"a".repeat(64),
            requested_zones: dataset_requested(&active, 2),
            successful_zone_series: vec![
                dataset_result_for_zone(&active, 0, &[0.0, 60.0]),
                dataset_result_for_zone(&active, 1, &[0.0, 60.0]),
            ],
            per_zone_failures: Vec::new(),
            forced_status: None,
        },
    ...
    let store = DesktopProjectSessionStore::default();
    {
        ...
        state.active_result_dataset = Some(dataset.clone());
    }
    let selection = AiAnalysisSelection {
        intent: "diagnose_run_result".into(),
        result_dataset_fingerprint: Some(dataset.dataset_fingerprint.clone()),
        metric: Some("temperature_k".into()),
        selected_time_seconds: Some(60.0),
    };
    let context = store
        ...
        .unwrap();
    assert_eq!(
        context.payload.pointer("/assistant_request/intent"),
        Some(&json!("diagnose_run_result"))
    );
    assert_eq!(
        context
            .payload
            .pointer("/result_summary/selected_values/0/value"),
        Some(&json!(294.15))
    );
    assert!(context.payload.pointer("/result_summary/samples").is_none());
    assert!(serde_json::to_string(&context)
        .unwrap()
        .find(active.source_path.to_string_lossy().as_ref())
        .is_none());

    let stale = AiAnalysisSelection {
        result_dataset_fingerprint: Some("b".repeat(64)),
        ..selection.clone()
    };
    assert_eq!(
        store
    ...
    let invalid = AiAnalysisSelection {
