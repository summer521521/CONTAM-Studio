...
前端新增且只新增`echarts 6.1.0`，使用`echarts/core`模块化注册`LineChart`、网格、提示、图例、数据缩放、轴指示、ARIA和Canvas renderer。单个ECharts实例绘制温度K、参考压力Pa和空气密度kg/m³三条纵向曲线，共享累计时间范围和`dataZoom`。曲线明确关闭平滑、采样、缺失值连接和动画；Tooltip从原始样本索引取值。
