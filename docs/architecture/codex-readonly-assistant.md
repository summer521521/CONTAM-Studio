...
项目打开、Zone切换或Revision变化只会使已披露上下文和Thread失效，不会把一个健康的App Server当作项目资源而重启。反之，Rust会在复用目录、账号或模型目录前无阻塞检查子进程是否仍存活；已退出、不完整或已断开的连接会清除其旧目录、账号和模型缓存，并在用户下一次明确点击“连接Codex”时创建新的受控会话，不能把失效目录伪装为可用连接。

`pnpm tauri dev`的首次窗口显示仍可能受Vite和Rust开发构建影响；这与用户主动连接后的本地App Server启动、账号读取、模型目录请求及联网模型响应是两段独立等待。窗口配置使用与工作台一致的背景色减少开发加载时的白色闪屏，但不把网络或服务端额度等待伪装成离线即时连接。
