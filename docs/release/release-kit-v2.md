...
便携 ZIP 内必须同时包含 `CONTAM-Studio.exe`、`runtime/python-worker/contam-studio-python-worker.exe`、`runtime/python-worker/_internal/`、运行时清单、许可证和第三方声明。

## 一致性要求

- `package.json`、Python、Cargo 和 Tauri 版本均为 `0.2.0`。
- `v0.2.0` 标签、Worker 清单、主程序构建、安装器清单和 GitHub Release 必须指向同一 release commit。
