...
- Tauri原生文件选择器只在用户主动操作时选择单个`.prj`；capability在既有`core:default`外仅增加`dialog:allow-open`，未开放前端文件系统、Shell、HTTP、远程URL或自动更新能力。
- React不读取文件、不启动进程；Rust宿主把文件路径作为JSON请求字段，通过参数数组启动一次性Python进程，并将结构化Envelope返回前端。
...
- Rust端超时为10秒，stdout上限2 MiB、stderr上限16 KiB；超时终止进程，并拒绝非UTF-8、无效JSON、协议不匹配、request_id不匹配、非零退出和超大输出。
...
- 新增Node依赖`@tauri-apps/api 2.11.1`、`@tauri-apps/plugin-dialog 2.7.1`和测试依赖`vitest 4.1.10`；新增Rust依赖`serde 1.0.228`、`serde_json 1.0.150`和`tauri-plugin-dialog 2.7.1`。相关项目均为MIT、Apache-2.0或双许可证，来自持续维护的Tauri、Serde和Vitest官方仓库。
...
- 前端Vitest测试11项通过，覆盖状态转换、一般错误、旧响应、零Zone、多Zone、Zone选择、属性、只读提示和双语错误文案；前端构建通过。
- Cargo测试15项通过，覆盖解释器发现、请求/响应契约、进程启动失败、非零退出、超时、非UTF-8、无效JSON、输出限制和实际官方fixture；Cargo检查通过。
