...
已接受；0.2.0的冻结分发与Windows进程树部分由[ADR-017](ADR-017-package-frozen-worker-and-own-windows-process-trees.md)扩展。
...
- 桌面开发运行可依赖项目Python虚拟环境；自0.2.0起安装包使用随包冻结Worker，本条原发布限制已由ADR-017关闭。
...
- Windows安装包内Python运行时的冻结、来源、许可和定位已由ADR-017实现；Authenticode签名、自动升级和独立干净机证据仍单独记录。
- 大型受支持PRJ的启动开销、内存、10秒超时和2 MiB响应上限是否合适。
- 主动取消和应用退出时进程树清理已由ADR-017的Windows Job Object实现；并发读取的长期策略仍待后续证据。
- 后续仿真执行是否复用相同Envelope但采用独立运行工作区和更长生命周期。
- 协议兼容测试、版本迁移和打包后的路径隐私策略。

## 后续实现状态

Phase 3B在不改变一次性进程决策的前提下将协议升级为`1.1`，显式增加Zone体积Patch计划和应用两个操作。读取/计划仍使用10秒超时，应用使用独立15秒超时，请求上限提高到128 KiB以容纳完整Patch。Rust持有活动项目和完整Patch，WebView不能提交路径或Patch对象；三个桌面命令均继续通过显式ACL开放。该实现仍不是长期sidecar或权限沙箱。
