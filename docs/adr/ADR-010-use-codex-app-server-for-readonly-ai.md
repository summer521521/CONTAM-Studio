...
Phase 6A-Beta-2增加默认关闭、用户明确启用的本地已完成问答档案。Rust只在应用本地数据目录保存通过既有结构化回答契约的安全字段，并以基线SHA-256和稳定Zone UUID过滤可见记录；档案不保存Thread、认证、原始协议、路径、PRJ正文、运行/结果文件或完整样本序列。档案只供用户查看和删除，绝不自动重新注入模型上下文或恢复旧Thread。关闭档案只停止后续保存，历史记录由用户明确删除。
...
- Phase 6A不支持本地模型、OpenAI Compatible API、Ollama、其他云提供方、跨重启Thread或自动聊天恢复、完整结果序列、工具或写入；Beta-2的可选本地档案不是Thread持久化，也不用于自动重放或模型上下文。
