...
- Python保留Phase 5A全部测试，并直接调用`extract_zone_air_state()`验证当前项目路径或SHA-256不匹配时返回`result_project_mismatch`，且不会probe或启动SimRead，也不会创建提取目录或改变源证据。
- 前端测试覆盖真实`selecting → loading`阶段、旧request通知、首次取消、保留旧结果后的取消与失败提示、项目/Zone切换清理、路径边界和真实样本表格。
- Rust测试覆盖Zone编号、名称、源行号、Schema、运行/提取ID、样本数量、有限数值、时间顺序和`day_type=null`契约，以及45秒提取超时常量。阶段事件仅含`request_id`和固定阶段；未开放dialog前端、fs、shell或HTTP权限。

## 真实验证记录

2026-07-18在真实Windows Tauri窗口完成验证并进行收口回归。打开官方`test_GetPrjInfo.prj`并选择Zone 1 `One`后，点击加载立即显示“正在选择Phase 4运行清单”；Rust取得有效JSON路径后界面切换为“正在加载运行结果”。NIST SimRead 3.4.0.3从Phase 4成功运行`20260718T041511Z-426b4cfa`提取并在桌面显示577个样本。首样本为0秒、293.15 K、-1.4222 Pa、1.2041 kg/m³，`day_type=null`，表格可纵向滚动。

- 原生manifest对话框取消后没有错误，已有同Zone的577个样本保持可用，并显示非破坏性的“本次加载已取消；仍显示上一次成功结果”提示。
