...
用户于2026-07-19完成首轮真实Tauri验收：清单中的步骤2至8、11、13至30通过，覆盖真实577样本、Tooltip、图表/表格切换、CSV导出、覆盖拒绝、取消提示、中英文和双主题。首轮反馈发现三个展示问题：状态栏永久显示ContamX未配置、只有第三张曲线显示累计时间横轴且存在不需要的拖放滑块、统计卡极值时间缺少明确标签。

当前收口已改为由运行状态驱动ContamX文案，三张曲线各自显示等长同范围横轴并只保留鼠标滚轮同步缩放，统计卡明确标注极值时间。用户已完成修正后的真实Tauri复核，GUI状态为`passed`；证据见[PR #13评论](https://github.com/summer521521/CONTAM-Studio/pull/13#issuecomment-5013358557)。未更新GUI截图。用户在fixture目录生成的CSV是手动验收产物，保持未跟踪且不纳入提交。
