...
export type SemanticOperationKind = "set_zone_volume" | "set_zone_name" | "set_flow_path_multiplier" | "set_flow_path_coefficient";
...
export interface SemanticNode { object_id?: string; zone_id?: string; level_id?: string; path_id?: string; species_id?: string; source_id?: string; object_kind?: string; name?: string; label?: string; fields?: Record<string, unknown>; capabilities?: Record<string, { state: string; unit: string | null; reason?: string }>; editable?: boolean; source_line_number?: number; section?: string; [key: string]: unknown; }
export interface SemanticSnapshot { result_type: "semantic_project_snapshot"; source_sha256: string; identity_sha256?: string; revision_state: string; project: SemanticNode; levels: SemanticNode[]; zones: SemanticNode[]; flow_paths: SemanticNode[]; schedules: SemanticNode[]; species: SemanticNode[]; sources: SemanticNode[]; sections: SemanticNode[]; document_envelope?: Record<string, unknown>; unknown_content?: { preserved: boolean; reason: string }; read_only_reason: string | null; [key: string]: unknown; }
...
export function semanticReducer(state: SemanticState, action: { type: string; [key: string]: unknown }): SemanticState {
  switch (action.type) {
    case "snapshot_loading": return { ...state, status: "loading", issue: null };
    ...
    case "context_changed": return { ...state, plan: null, status: state.operations.length ? "editing" : state.status };
