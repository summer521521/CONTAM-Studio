...
export function SemanticPropertyPanel({ node, selectedNodes = [], state, onEdit, onUndo, onRedo, onPlan, onApply, onDiscard }: Props) {
  ...
  return <section className="semantic-property-panel" aria-label={t("semantic.properties")}>
    ...
    <dl className="property-list">
      ...
      {editable("name") ? <div><dt>{t("inspector.name")}</dt><dd><input className="semantic-field-input" value={state.operations.find((item) => item.object_id === id && item.operation === "set_zone_name")?.new_value ?? currentValue(node, "name")} onChange={(event) => setField(nameOperation, event.target.value)} /></dd></div> : null}
      {editable("volume_m3") ? <div><dt>{t("inspector.volume")}</dt><dd><input className="semantic-field-input" inputMode="decimal" value={state.operations.find((item) => item.object_id === id && item.operation === "set_zone_volume")?.new_value ?? currentValue(node, "volume_m3")} onChange={(event) => setField(volumeOperation, event.target.value)} /><span className="field-unit">m³</span></dd></div> : null}
      {editable("multiplier") ? <div><dt>{t("semantic.multiplier")}</dt><dd><input className="semantic-field-input" inputMode="decimal" value={state.operations.find((item) => item.object_id === id && item.operation === "set_flow_path_multiplier")?.new_value ?? currentValue(node, "multiplier")} onChange={(event) => setField(multiplierOperation, event.target.value)} /></dd></div> : null}
    ...
    <div className="semantic-actions">
      <button type="button" className="secondary-action" onClick={onUndo} disabled={!state.undo.length}><RotateCcw size={14} />{t("semantic.undo")}</button>
      <button type="button" className="secondary-action" onClick={onRedo} disabled={!state.redo.length}><RotateCw size={14} />{t("semantic.redo")}</button>
