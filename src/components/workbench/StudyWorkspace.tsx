...
import { isSafeStudyPlan, isSafeStudySampleResult, studyReducer, INITIAL_STUDY_STATE, type StudyMode, type StudyParameter, type StudyPlan } from "../../app/study-state";
import type { ProjectInspection } from "../../app/project-state";
...
export function StudyWorkspace({ project, projectSessionId, revisionId, onNotice = () => undefined }: StudyWorkspaceProps) {
  ...
  async function loadPage(plan: StudyPlan, nextPage: number) {
    ...
    const numericFilter = filterValue.trim() === "" ? null : Number(filterValue);
    const response = await pageStudyResults(
      ...
      8,
      "zone-volume",
      numericFilter !== null && Number.isFinite(numericFilter) ? numericFilter : null,
      null,
      null,
      sortBy,
      false,
  ...
  async function createPlan() {
    ...
    try {
      ...
      if (!isSafeStudyPlan(planValue)) {
        dispatch((current) => ({ ...current, issue: response.error ?? { code: "study_response_invalid", message: "Study plan response invalid", source_line_number: null, context: {} }, status: "failed" }));
        return;
      }
  ...
  async function runPlan() {
    ...
    try {
      ...
      if (!response.error && response.result && Array.isArray(rawResults) && results.length === rawResults.length) {
        ...
        dispatch((current) => studyReducer(current, { type: "run_failed", requestId, issue: response.error ?? { code: "study_run_invalid", message: "Study run response invalid", source_line_number: null, context: {} } }));
      }
    } finally {
      setBusy(false);
    }
