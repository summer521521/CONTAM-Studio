...
def classify_project(path: Path, *, companions: tuple[CompanionDeclaration, ...] = ()) -> CompatibilityResult:
    ...
    species: tuple[SpeciesProjection, ...] = ()
    species_section = sections.section("species")
    ...
    safe_action = "仅可检查已投影对象，先创建副本后再请求支持的Patch。" if status == CompatibilityStatus.SUPPORTED_READONLY else "可对受支持的Zone字段生成Patch Diff。"
